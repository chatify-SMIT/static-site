...
function Forgotpassword() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        let { status, data } = await verifyOTP({
    ...
    <form onSubmit={formik.handleSubmit}>
      ...
      <div className="d-flex flex-column content-login d-flex justify-content-center  align-items-center">
        ...
        <div className="d-flex flex-row justify-content-start w-100">
          ...
          />
          <button
            className="btn  btn-primary fw-bold mt-3 w-25 mx-2 otp"
            type="button"
        ...
        <input
          ...
          type="password"
        />
        <button className="btn btn-primary fw-bold mt-3 w-100" type="submit">
          Reset Password
        </button>
        <Link to="/">
          {" "}
          <div className="d-flex align-items-center fw-bold pb-2 mt-3 text-dark text-decoration-none">
            Return to Log in
          </div>{" "}
        </Link>
      </div>
